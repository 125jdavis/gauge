...
// vBatt, on pin 0
float vBatt;
int filter_vBatt = 3; // out of 16, 16 = no filter
int vBattPin = A0;
float vBattScaler = 4; // voltage divider factor (in this case r1 = 10k, r2 = 3.3k)
...
void loop() {
  ...
  if (millis() - timerSensorRead > sensorReadRate) {
    ...
    vBatt = readSensor(vBattPin, vBatt, filter_vBatt)*vBattScaler;
